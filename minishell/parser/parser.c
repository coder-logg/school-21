...
static char	*replace_tilda(char **cmd, char **env, int i)
{
	int		index;
...
static char	*parse_line(char *cmd, char **env)
{
	int		i;
...
static int	shrink_spaces(char *const *line, int i)
{
	if ((*line)[i] == '\'' || (*line)[i] == '"')
...
static int	preparser(char **l, bool flag)
{
	int	i;
